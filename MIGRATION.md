...
1. **Master Key** - Used by Aegis to ENCRYPT secrets for a host
   ...
   - The public key is stored in `nix-entities` or set via `aegis host set-key`
   - Aegis uses this to encrypt secrets only that host can decrypt

2. **SSH Host Keys** - Used by OpenSSH to identify the server
   - Generated by `aegis build ssh-keys` or imported via `aegis ssh import`
...
aegis host set-key lambda --public-key "ssh-ed25519 AAAAC3Nza... lambda-master"
```
...
```bash
aegis secret import <hostname> <secret-name> \
...
**Example:**
```bash
aegis secret import lambda postgresql-password \
...
aegis host set-key <host> --public-key "ssh-ed25519 AAAA..."
...
aegis ssh import <host> --key <path> --key <path>

# Nexus keys
aegis nexus import <host> --file <path>

# Kerberos realms
aegis realm import <REALM> --realm-key <path> --principals-dir <path>

# Generic secrets
aegis secret import <host> <name> --file <path> --target <path> --user <user> --group <group> --mode <mode>
```
...
aegis build ssh-keys        # Just OpenSSH host keys
