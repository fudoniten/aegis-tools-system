...
# Rotate the key of an existing principal, bumping its kvno.
#
# Writes the principal's new dump line to PRINCIPAL_DIR, replacing whatever
# was there. The caller is responsible for archiving the previous line first
# if it wants a graceful rotation: a keytab can hold several kvnos for the
# same principal, so keeping the old key around lets services keep
# authenticating until every one of them has the new keytab.
...
raise "principal does not exist: #{output_filename}" unless File::exist?(output_filename)
...
principal_keys = Dir::mktmpdir("rekey_principal") do |tmpdir|
  pw_clause = if options[:password]
                "--password=\"#{options[:password]}\""
              else
                "--random-key"
              end
  exec!(verbose, "Rotating key for #{principal} ...",
        ["kadmin",
         "--local",
         "--config-file=#{config}",
         "--",
...
raise "principal vanished from the database after rekey: #{principal}" if principal_keys.empty?

print("Writing rotated principal key to #{output_filename} ... ") if verbose
